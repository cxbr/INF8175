    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextX][nextY]:
                ...
                updatedCornersVisited = list(cornersVisited)
                if nextPosition in self.corners:
                    cornerIndex = self.corners.index(nextPosition)
                    updatedCornersVisited[cornerIndex] = True
                successors.append(
                    ((nextPosition, tuple(updatedCornersVisited)), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    unvisitedCorners = [corners[i]
                        for i, visited in enumerate(visitedCorners) if not visited]
...
def foodHeuristic(state, problem: FoodSearchProblem):
    ...
    for food in foodList:
        positionSearchProblem = PositionSearchProblem(
            problem.startingGameState, start=position, goal=food, warn=False, visualize=False)
        distances.append(len(search.breadthFirstSearch(positionSearchProblem)))
